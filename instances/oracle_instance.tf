...
output "oracle_instance_private_ip" {
  ...
  value       = length(aws_instance.oracle_db_instance) > 0 ? aws_instance.oracle_db_instance[0].private_ip : "Oracle instance not deployed (count=0)"
}

output "oracle_access_command" {
  description = "Command to access Oracle instance via bastion"
  value       = length(aws_instance.oracle_db_instance) > 0 ? "ssh -i ${var.bastion_ssh_key_path} -J ubuntu@${aws_instance.bastion[0].public_ip} ubuntu@${aws_instance.oracle_db_instance[0].private_ip}" : "Oracle instance not deployed (count=0)"
}
