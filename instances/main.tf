...
# Security Group for the K8s Load Balancer (must be defined BEFORE k8s_sg)
...
# Security Group for Kubernetes Cluster (Master & Workers) - SINGLE DEFINITION
...
resource "aws_security_group" "rds_sg" {
  ...
  # Allow from VPC CIDR (broadest rule for debugging)
  ingress {
    from_port   = 3306
    to_port     = 3306
    protocol    = "tcp"
...
resource "aws_db_instance" "clixx_db" {
  ...
  snapshot_identifier    = var.db_snapshot_identifier  # ✅ Reference original snapshot directly
...
# ========================================
# LOAD BALANCER RESOURCES (Consolidated)
# ========================================
...
# Target Group for NodePort 30000 (matching clixx-service.yaml)
...
resource "aws_lb_listener" "k8s_https" {
  ...
  certificate_arn   = data.aws_acm_certificate.clixx_cert.arn  # ✅ This references acm_verification.tf
...
# This is crucial since your Docker image has hardcoded URL
